/**
 * 对单个文件的 Update 块应用行替换。
 * 逻辑来自 openclaw-cn src/agents/apply-patch-update.ts，仅保留 applyUpdateHunk 及依赖，无 Node fs 默认依赖。
 */
...
function seekSequence(
  ...
  for (let i = searchStart; i <= maxStart; i += 1) {
    if (
      linesMatch(lines, pattern, i, v => normalizePunctuation(v.trim()))
    )
      return i
  }
  return null
