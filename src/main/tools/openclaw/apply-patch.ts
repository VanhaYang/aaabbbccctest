/**
 * apply_patch 工具：解析并应用 *** Begin Patch / *** End Patch 格式。
 * 逻辑与格式来自 openclaw-cn src/agents/apply-patch.ts，此处仅依赖 DesktopBridge，不依赖 Gateway/sandbox。
 */
import * as path from 'path'
import type { DesktopBridge } from './adapter'
import { isPathInside } from '../../pathGuards'
import { applyUpdateHunk } from './apply-patch-update'
import type { UpdateFileChunk } from './apply-patch-update'
...
function buildFileOps(bridge: DesktopBridge): PatchFileOps {
  ...
  return {
    ...
    remove: (filePath: string) => bridge.remove(filePath),
    mkdirp: (dir: string) => bridge.mkdirp(dir)
  }
}
...
function resolvePatchPath(
  ...
  const display =
    !relative || relative === '' ? path.basename(resolved) : relative
  if (display.startsWith('..') || path.isAbsolute(display)) {
    return { resolved, display: resolved }
  }
  return { resolved, display }
}

async function ensureDir(filePath: string, ops: PatchFileOps): Promise<void> {
...
export async function applyPatch(
  ...
  const cwd = bridge.getWorkspaceRoot()
  const fileOps = buildFileOps(bridge)
  ...
  for (const hunk of parsed.hunks) {
    ...
    const applied = await applyUpdateHunk(target.resolved, hunk.chunks, {
      readFile: (p: string) => fileOps.readFile(p)
    })
...
function parseOneHunk(
  ...
  throw new Error(
    `Invalid patch hunk at line ${lineNumber}: '${lines[0]}' is not a valid hunk header. Valid: *** Add File: {path}, *** Delete File: {path}, *** Update File: {path}`
  )
}
...
function parseUpdateFileChunk(
  ...
  if (startIndex >= lines.length) {
    throw new Error(
      `Invalid patch hunk at line ${lineNumber + 1}: Update hunk has no lines`
    )
  }
  ...
  for (const line of lines.slice(startIndex)) {
    ...
    if (parsedLines === 0) {
      throw new Error(
        `Invalid patch hunk at line ${lineNumber + 1}: Unexpected line: '${line}'`
      )
    }
    break
  }
